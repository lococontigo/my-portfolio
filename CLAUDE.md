...
**See `DESIGN.md` for the full design-system reference** (YAML token definitions plus color roles, type hierarchy, component states, spacing/depth rules, and responsive behavior in prose) — this file covers technical/dev conventions, `DESIGN.md` covers visual/design-system rules. Keep them in sync: a token, component state, or breakpoint change belongs in both if it affects how UI should look.
